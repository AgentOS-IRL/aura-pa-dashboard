...
The new OpenAI client and configuration helpers live under `backend/src/services/openaiClient.ts` and `backend/src/config/openai.ts`, and `npm run test:backend` covers their validation so the runtime fails fast if the credentials or model settings are missing.
