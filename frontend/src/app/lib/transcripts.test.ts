...
describe("fetchTranscripts", () => {
  ...
  it("includes the unclassifiedOnly flag when requested", async () => {
    const fetchMock = vi.fn(() =>
      Promise.resolve({
        ok: true,
        json: () => Promise.resolve({
    ...
    await fetchTranscripts({ limit: 5, unclassifiedOnly: true });

    expect(fetchMock).toHaveBeenCalledWith("http://localhost:4000/aura/transcripts?limit=5&unclassifiedOnly=true", {
      method: "GET",
      signal: undefined
    });
  });
