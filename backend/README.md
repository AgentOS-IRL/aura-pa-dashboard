...
- When you set `?unclassifiedOnly=true` (without also sending `classificationId` or `classificationState`), the route instead returns transcripts that currently have no entries in `transcript_classifications`, giving you the true “unclassified only” badge count and page payload.
